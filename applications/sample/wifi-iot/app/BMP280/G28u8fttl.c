...
#define GPS_UART_BAUDRATE           115200                //串口通讯波特率
...
void NMEA_GNGGA_Analysis(nmea_msg *gpsx,uint8_t *buf)
{
	...
	// // not used 
	(void)f_latitude, (void)f_longitude;
	(void)posx, (void)dx;
	(void)temp, (void)rs;
	...
	latitude = strsep((char **)&p1, ",");					//获取纬度
	nshemi = strsep((char **)&p1, ",");						//获取南北纬
	longitude = strsep((char **)&p1, ",");					//获取经度
	ewhemi = strsep((char **)&p1, ",");						//获取东西经
	gpssta = strsep((char **)&p1, ",");						//获取定位状态
...
/******************************************************************************/ 
{
	...
	// uint8_t data;
	// uint16_t i;
    uint8_t recv_data_buf[512] = {0};
    uint8_t send_data_buf[512] = {0};
    ...
    if(GPS_Init())
		printf("suceed");  //initialize the BMP280
	...
	hi_uart_write(GPS_UART_NUM, &hot_open[0][0], 12);


    while (1)
    {

		hi_s32 len = hi_uart_read_timeout(GPS_UART_NUM, recv_data_buf, sizeof(recv_data_buf),600);
		if (len > 0) {
			...
			printf("h: %d, m: %d, s: %d", s_GpsMsg.utc.hour, s_GpsMsg.utc.min, s_GpsMsg.utc.sec);
		} 
