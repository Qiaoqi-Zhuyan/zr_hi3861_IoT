...
int mqtt_connect(void)
{
	...
	char clientId[64];
	sprintf(clientId,"RoomDet/%s", ROOM_ID);
	...
	/* subscribe 订阅主题 */
	char sub_topic[64];
	sprintf(sub_topic,"%s/device_sub", ROOM_ID);
	...
	while (1)
	{
		...
		usleep(100000);
		char pub_topic[64];
		sprintf(pub_topic,"%s/env_pub", ROOM_ID);
		...
		*/
		...
		if(packet.id == 0x2B){
			sprintf(payload, "{\"id\" : %d, \"room\": \"%s\", \"data\": %s}", packet.id, ROOM_ID ,packet.data);
		}
