...
{
    WifiIotI2cData buffer[2]={uch_addr,uch_data};
    if(I2cWrite(MAX30102_I2C_IDX, (MAX30102_ADDR<<1)|0, buffer)){
 ...
 * @author yuchen
 */
static unsigned int maxim_max30102_read_reg(WifiIotI2cData uch_addr, WifiIotI2cData *data, uint16 len)
{
    unsigned int ret=0;
	(void)len;
    ret+=I2cWrite(MAX30102_I2C_IDX,(MAX30102_ADDR<<1)|0,&uch_addr);
    ret+=I2cRead(MAX30102_I2C_IDX,(MAX30102_ADDR<<1)|1,data);
    if(ret)
...
void max30102_read_fifo(void)
{
	...
	fifo_red=0;
	fifo_ir=0;
	WifiIotI2cData ach_i2c_data[6];
	WifiIotI2cData uch_temp;
	unsigned int ret=0;
...
void MAX30102Task(void *arg)
{
    ...
    hi_io_set_func(3, HI_IO_FUNC_GPIO_3_I2C1_SDA);
    hi_io_set_func(4, HI_IO_FUNC_GPIO_4_I2C1_SCL);
