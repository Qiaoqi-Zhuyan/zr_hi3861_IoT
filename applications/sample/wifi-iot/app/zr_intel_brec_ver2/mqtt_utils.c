...
int mqtt_connect(void)
{
	...
	char clientId[64];
	sprintf(clientId,"intel_brec_dev/%d", DEVICE_ID);

	data.clientID.cstring = clientId; // 修改成自己名称，
	...
	while (1)
	{
		...
		// 发送数据
		char pub_topic[64];
		sprintf(pub_topic, "brec_pub/%d", DEVICE_ID);
		topicString.cstring = pub_topic; // 发布设置主题

		char payload[256];
		if(blood_packet.id != 0x00){
			sprintf(payload, "{\"id\" : %d, \"device_id\": %d ,\"data\": %s}", blood_packet.id, DEVICE_ID ,blood_packet.data);
		...
		if (temp_packet.id != 0x00){
			sprintf(payload, "{\"id\" : %d, \"device_id\": %d ,\"data\": %s}", temp_packet.id, DEVICE_ID ,temp_packet.data);
			int payloadlen = strlen(payload);
			len = MQTTSerialize_publish(buf, buflen, 0, 0, 0, 0, topicString, (unsigned char *)payload, payloadlen);
			transport_sendPacketBuffer(mysock, buf, len);
			temp_packet.id = 0x00;
