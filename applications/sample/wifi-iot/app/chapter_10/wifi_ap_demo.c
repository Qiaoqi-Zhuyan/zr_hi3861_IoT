...
static void WifiAPTask(void *arg)
{
    ...
    rc = memcpy_s(hapd_conf.ssid, HI_WIFI_MAX_SSID_LEN + 1, SSID, 16); /* 9:ssid length */
    if (rc != EOK)
    {
        return ;
    }
    ...
    rc = memcpy_s(hapd_conf.key, HI_WIFI_MAX_KEY_LEN + 1, PASSWORD, 9); /* 9:ssid length */
    if (rc != EOK)
    {
        return ;
    }
